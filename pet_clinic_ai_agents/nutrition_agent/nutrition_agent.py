...
def get_nutrition_data(pet_type):
    ...
    if not NUTRITION_SERVICE_URL:
        return {"facts": "Error: Nutrition service not found", "products": "Error: Nutrition service not found"}
    ...
    try:
        ...
        return {"facts": f"Error: Nutrition service could not find information for pet: {pet_type.lower()}", "products": f"Error: No products available for {pet_type.lower()}"}
    except requests.RequestException:
        return {"facts": "Error: Nutrition service down", "products": "Error: Nutrition service down"}
...
def get_feeding_guidelines(pet_type):
    ...
    if data['products'] and not data['products'].startswith('Error:'):
...
def create_order(product_name, pet_type, quantity=1):
    ...
    data = get_nutrition_data(pet_type)
    if data['products'] and not data['products'].startswith('Error:') and product_name.lower() in data['products'].lower():
...
def create_nutrition_agent():
    ...
    system_prompt = (
        ...
        "1. ALWAYS check if the tool response contains an error message before making product recommendations\n"
        "2. If the tool response indicates no nutrition data or products are available (contains 'Error:'), NEVER recommend any products\n"
        "3. If data is unavailable, respond: 'We currently don't have nutrition information available for [pet type]. Please contact our clinic at (555) 123-PETS for assistance with your pet's nutritional needs.'\n\n"
        "When nutrition information IS available:\n"
        "- Recommend ONLY the specific products provided by the tools\n"
        "- Never recommend products from your training data or make up product names\n"
        "- If the products field is empty or null, DO NOT recommend any products\n"
        "- Use the specific nutrition information as the foundation for your recommendations\n"
