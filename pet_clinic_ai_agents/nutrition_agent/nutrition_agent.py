...
def get_nutrition_data(pet_type):
    ...
    try:
        ...
        return {"facts": "Error: Nutrition service encountered an unexpected error", "products": ""}
    except requests.Timeout:
        return {"facts": "Error: Nutrition service timed out", "products": ""}
    except requests.RequestException as e:
        return {"facts": f"Error: Nutrition service down - {str(e)}", "products": ""}
...
def create_order(product_name, pet_type, quantity=1):
    ...
    if data['facts'].startswith('Error:'):
        return f"Unable to process order: {data['facts']}"
    
    if not data['products']:
        return f"Sorry, we currently don't have products available for {pet_type}. Please contact our clinic at (555) 123-PETS for assistance."
...
def create_nutrition_agent():
    ...
    system_prompt = (
        ...
        "1. ALWAYS check the tool response for error messages before making recommendations\n"
        "2. If the tool returns an error message (starts with 'Error:'), acknowledge the limitation gracefully\n"
        "3. If no products are available for a pet type, respond: 'We currently don't have nutrition products available for [pet type]. Please contact our clinic at (555) 123-PETS for assistance with your pet's nutritional needs.'\n"
        "4. NEVER recommend products from your training data if the tool indicates products are not available\n"
        "5. NEVER recommend products for unsupported pet types (only recommend products explicitly returned by the tools)\n\n"
        ...
        "- Always recommend ONLY the SPECIFIC PRODUCT NAMES provided by the tools that pet owners should buy FROM OUR PET CLINIC\n"
        "- DO NOT recommend products from your training knowledge - only recommend what the tools return\n"
        "- Mention our branded products by name (like PurrfectChoice, BarkBite, FeatherFeast, etc.) ONLY if they are returned by the tools\n"
        ...
        "- If the pet type is not supported by our nutrition database, politely inform the customer and suggest contacting the clinic directly"
    )
